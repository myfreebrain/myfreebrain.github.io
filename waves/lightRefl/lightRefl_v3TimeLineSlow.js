...
if(rayRX>=reflRX){posBeta = rayRX+10;}
          ...
          var noOfDivs = lengthMLine/divM/2;//half the number of divs
                        ...
                        if(textFlag){
                          ...
                          ctx.font = "10px serif";
                          ctx.moveTo(minX, mirrorLY);
                          ctx.lineTo(minX-10, mirrorLY);
                          ctx.stroke();
                          var posMLs = ((maxY-mirrorLY)/50).toFixed(2);
                          ctx.fillText(""+posMLs, minX-37, mirrorLY+3);
                          //left ridge
                          ctx.moveTo(maxX, mirrorRY);
                          ctx.lineTo(maxX+10, mirrorRY);
                          ctx.stroke();
                          var posMRs = ((maxY-mirrorRY)/50).toFixed(2);//((posAY-minY)/50).toFixed(2);
                          ctx.fillText(""+posMRs, maxX+14, mirrorRY+3);
                      }
