...
var arrColor = new Array();//hold color of arrow - 0 = black, 1=orange
var color0 = "black";
var color1 = "orange";
var arrLength = 10;//half length of arrow
var arrNewLength = new Array();
var gridC = 37;//x - columns
var gridR = 18;//y - rows
var gridX = (canvW-60)/gridC;
var gridY = (canvH-90)/gridR;
var miceX = 0;
var miceY = 0;
var equipotFlag = false;
var constMagFlag = true;
...
window.onload = function(){
  ...
  if(drawDotsF){document.getElementById("noLines").checked=true;}
    ...
    ctx.moveTo(c*gridX+30, 61);
    ctx.lineTo(c*gridX+30, canvH-31);
    ctx.stroke();
    }
    for(c=1;c<gridR+1;c++){
    ctx.beginPath();
    ctx.lineWidth = "1";
    ctx.strokeStyle = "silver";
    ctx.moveTo(31, c*gridY+60);
    ctx.lineTo(canvW-31, c*gridY+60);
    ctx.stroke();
    }
    ...
    if(dragEnabled){
      ...
      if(Qx>xmin && Qx<xmax && Qy>ymin && Qy<ymax){
        ...
        arrYA[noOfArr]=Qy;
        if(!equipotFlag){
          arrColor[noOfArr]=color0;
        }
        else{
          arrColor[noOfArr]=color1;
        }
        noOfArr++;//added for compass
        //newQflag=false;
        plotNewArrow();
...
function plotNewMoveQ(){
 ...
 if(oldQflag){
  ...
  ctx.fill();
      ...
      ctx.closePath;
miceX=Math.round(Qx-30);
miceY=Math.round(Qy-60);
            document.getElementById("dummy1").innerHTML="x="+miceX + " y="+miceY;
}
...
function plotNewArrow2(){
      ...
      //dot in centre
            ...
            ctx.fillStyle = "black";
            ctx.fill();
            /*CODE TO DRAW COMPASS OUTLINE CIRCLE
            ctx.beginPath();
            ...
            */
            ...
            findFieldXY(Qx, Qy , nowArr);
                          var arr2Length = arrNewLength[nowArr];//-2 to make compass a bit smaller than circle
...
function changeDrag(){
    if(dragEnabled){
      ...
      findFieldF=true;
      equipotFlag=true;
      document.getElementById("dragB").value="find equipotentials";
    }
    else{
          findFieldF=false;
          dragEnabled=true;
...
function findFieldXY(x,y,k){
  ...
  var totalY = 0;
    ...
    if(equipotFlag){
       arrAngle[arrowNo] = Math.atan(totalY/totalX)+Math.PI/2;//+Math.PI/2; for equipotentials
     }
    else{
       arrAngle[arrowNo] = Math.atan(totalY/totalX);
    }
    var force = Math.sqrt(Math.pow(totalX,2)+Math.pow(totalY,2));//10 IS 2ND CONSTANT
    if(constMagFlag){
      arrNewLength[arrowNo] = 10;
    }
    else{
    arrNewLength[arrowNo] = Math.pow(force,0.25);
    }
