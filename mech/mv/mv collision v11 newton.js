...
function calcChangeAandB(){
  ...
  //setTimeout(dummyPause, 2000);
    ...
    objVelA = (objMassA*oldVelA - elasticity*(objMassB*oldVelA)+(1+elasticity)*(objMassB*oldVelB))/(objMassA+objMassB);
    objVelB = elasticity*oldVelA - elasticity*oldVelB + objVelA;
...
if(goFlag){
  ...
  if(ballDiff<=collisionDist&&!afterCollFlag){
    collisionFlag = true;
    if(springT0==0){
      springT0=time;//time of 1st contact
        springTr0=turn;
  ...
  if(collisionFlag&&oneMoreFlag){
    ...
    if(collisionFlag){//&&!oneMoreFlag&&springX>=0
        //calcCollisionAonB();//if using SPRING model
        //setTimeout(calcChangeAandB, 1000);
        calcChangeAandB();//if using NEWTON model
