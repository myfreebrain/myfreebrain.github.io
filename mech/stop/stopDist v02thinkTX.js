...
for(k=0; k<10;k++){
  numbersActive[k] = numberA[k];
  numbersLen = numbersActive.length;
}
...
function findWord(){
  ...
  for(k=0; k<10;k++){
	...
	for(j=0; j<wordLen; j++){
		letNow = Math.floor(Math.random()*numbersLen);
		letterA[j] = numbersActive[letNow];
    numbersActive[letNow] = numbersActive[numbersLen-1];
    numbersLen--;
		fullWord=fullWord+letterA[j];
	}
	//fullWord=fullWord+"!";
  if(Math.random()<0.1){
    var dim1=Math.floor(Math.random()*wordLen);
    var dim2=Math.floor(Math.random()*wordLen);
    //alert(""+dim1+" "+dim2+" : "+letterA);
    if(dim1==dim2){
      //do nothing
    }
    else{
      letterA[dim2]=letterA[dim1];
      fullWord="";
      for(k=0;k<wordLen;k++){
  		fullWord=fullWord+letterA[k];
      }
